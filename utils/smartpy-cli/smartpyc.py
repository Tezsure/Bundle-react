...
def run_tests(in_py):
    ...
    for test in browser.window.pythonTests:
        ...
        scenarios.append({'shortname': test.shortname, 'longname': test.name, 'scenario' : scenario, 'decompile' : test.decompile, 'kind' : test.kind})
    return json.dumps(scenarios)

def write_tests(out_scenario_sc, fn_py, fn_pure_py):
    code = write_pure_py(fn_pure_py, fn_py)
    run_script(fn_py, code, {'__name__' : '__main__'})
    scenarios = run_tests(fn_py)
    open(out_scenario_sc, "w").write(scenarios)

def write_with_init_expr(out_smlse, fn_py, fn_pure_py, init):
    code = write_pure_py(fn_pure_py, fn_py)
...
def write_with_init(out_smlse, fn_py, fn_pure_py, init):
    code = write_pure_py(fn_pure_py, fn_py)
    context = {'__name__' : '__main__'}
    run_script(fn_py, code, context)
